// Der Einstieg der Seite "merchants" (#154).
//
...
import { renderMerchants, bindMerchants, newMerchant } from './page.js';
...
await startShellPage(async context => {
  pageContext = context;
  if (!bound) {
    bindMerchants(context);
    bound = true;
  }
  await renderMerchants(context);
}, { primaryAction: () => ['merchants.new', () => newMerchant(pageContext)] });
