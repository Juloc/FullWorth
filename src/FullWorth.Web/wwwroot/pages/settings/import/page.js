...
const t={
  de:{subtitle:'Buchungen und Depots aus anderen Apps übernehmen.',back:'Zurück',space:'FullWorth Space',txHint:'Für Bank- und App-Exporte. CSV und XLSX werden zuerst analysiert; du bestätigst Spalten und Zielkonten vor dem Import.',invHint:'Für Parqet, Finanzfluss und andere Depot-Exporte. Käufe, Verkäufe, Dividenden, Zinsen, Gebühren und Steuern werden geprüft.',analyse:'Datei analysieren',review:'Import prüfen',import:'Importieren',importDepot:'Depot importieren',mapping:'Spalten zuordnen',targets:'Konten zuordnen',targetsHint:'Jedes in der Datei gefundene Konto bekommt ein Zielkonto in FullWorth.',securities:'Wertpapiere prüfen',working:'Wird verarbeitet …',needFile:'Bitte zuerst eine CSV- oder XLSX-Datei wählen.',needMapping:'Datum und Betrag müssen zugeordnet sein.',needInvestmentMapping:'Handelsdatum und Transaktionsart müssen zugeordnet sein.',needAccount:'Bitte alle Quellkonten einem Zielkonto zuordnen.',needPortfolio:'Bitte ein Zieldepot wählen.',done:'Import abgeschlossen.',error:'Import fehlgeschlagen.',rows:'Zeilen',ready:'Bereit',errors:'Fehler',imported:'Importiert',duplicates:'Duplikate',newPortfolio:'Neues Import-Depot anlegen',none:'Nicht zuordnen',autoNew:'Automatisch / neu anlegen',matched:'Automatisch erkannt',willCreate:'Wird neu angelegt',noAccounts:'Keine beschreibbaren Konten gefunden.',noPortfolios:'Noch kein Depot vorhanden.',detectTitle:'Datei importieren',detectHint:'Datei wählen — FullWorth erkennt selbst, woher sie kommt.',detectOr:'oder Quelle selbst wählen',detecting:'Datei wird gelesen …',detectContinue:'Weiter',detectUnknown:'Diese Datei lässt sich keiner Quelle sicher zuordnen. Wähle die Quelle bitte selbst.',detectFailed:'Die Datei konnte nicht gelesen werden.',srcFinanzguru:'Finanzguru-Export',srcStatement:'Kontoauszug',srcTransactions:'Buchungen (CSV/XLSX)',srcInvestments:'Depot (CSV/XLSX)',srcBrokerPdf:'PDF',srcUnknown:'Unbekannt',whyFinanzguru:'Die Arbeitsmappe hat die Kopfzeilen des Finanzguru-Exports.',whyMt940:'Die Datei ist ein MT940-Auszug.',whyCamt:'Die Datei ist ein CAMT-Auszug.',whyPdf:'Die Datei ist ein PDF, aber kein Kontoauszug, den FullWorth kennt – sie geht an den Depot-Import für Broker-Abrechnungen.',whyIkanoPdf:'Die Datei ist ein Kontoauszug der Ikano Bank (IKEA-Kreditkarte).',whyC24Pdf:'Die Datei ist ein Kontoauszug der C24 Bank.',whyInvestments:'Die Tabelle führt Wertpapierspalten.',whyTabular:'Die Tabelle hat eine Datums- und eine Betragsspalte.',whyUnmapped:'Die Tabelle hat weder eine erkennbare Datums- noch Betragsspalte.',whyUnreadable:'Kein Leser versteht diese Datei.',whyNoRows:'Die Datei enthält ausser der Kopfzeile keine Zeilen.',detectAccounts:'{n} Konten',detectRows:'{n} Buchungen',detectPeriod:'{from} – {to}',historyEmpty:'Noch keine Depotimporte.',rollback:'Import rückgängig machen',rollbackConfirm:'Diesen Depotimport wirklich rückgängig machen? Spätere abhängige Buchungen schützen den Rollback automatisch.',rolledBack:'Import wurde rückgängig gemacht.',healthy:'Plausibel',checkWarnings:'Hinweise',cash:'Cash',positions:'Positionen',rowsTitle:'Buchungen prüfen',rowsHint:'Wähle ab, was nicht importiert werden soll. Zeilen mit Fehlern sind nicht auswählbar.',selectAll:'Alle auswählen',selectedOf:'{n} von {total} ausgewählt',errorsTitle:'Zeile nicht importierbar',notImportable:'nicht importierbar',noRows:'Keine Zeilen gefunden.',showing:'Angezeigt werden die ersten {n} von {total} Zeilen. Die Auswahl gilt für alle.',noneSelected:'Bitte mindestens eine Buchung auswählen.',duplicate:'Schon vorhanden',dupInFile:'kommt in dieser Datei doppelt vor',dupExternalKey:'Buchungs-ID wurde schon importiert',dupExisting:'gleiches Datum, gleicher Betrag, gleicher Empfänger schon gebucht',dupProbable:'Vermutlich schon vorhanden: {name} am {date}, aus einer anderen Quelle. Anhaken, wenn es eine zweite Zahlung ist.',dupHint:'{n} Buchungen sind schon oder vermutlich schon vorhanden und deshalb abgewählt.',dupChecking:'Duplikate werden geprüft …',txHistory:'Letzte Buchungsimporte',txHistoryEmpty:'Noch keine Buchungsimporte.',txRollback:'Import rückgängig machen',txRollbackConfirm:'Diesen Import wirklich rückgängig machen? Buchungen, die du danach geteilt, verschlagwortet, geprüft oder mit einem Vertrag verknüpft hast, bleiben erhalten.',txRolledBack:'{removed} entfernt, {kept} behalten.',statusCompleted:'Abgeschlossen',statusCancelled:'Abgebrochen',statusFailed:'Fehlgeschlagen',statusRolledBack:'Rückgängig gemacht',categoryTargets:'Kategorien zuordnen',categoryTargetsHint:'Jede Kategorie aus der Datei bekommt eine FullWorth-Kategorie. „Automatisch" nimmt eine gleichnamige Kategorie oder überlässt es den Regeln.',autoCategory:'Automatisch',noCategoriesFound:'Die Datei enthält keine Kategoriespalte.',needStatementFile:'Bitte zuerst eine MT940- oder CAMT-Datei wählen.',needStatementAccount:'Bitte ein Konto wählen.',newAccount:'Neues Konto anlegen',newAccountName:'Name des neuen Kontos',newAccountDefault:'Importiertes Konto',needAccountName:'Bitte einen Namen für das neue Konto angeben.',stmtHint:'Für Konten, die FullWorth nicht direkt verbinden kann: MT940- oder CAMT-Datei, oder den PDF-Kontoauszug von Ikano oder C24 hochladen und das Konto wählen, zu dem er gehört.',stmtAccountFromFile:'Konto laut Datei: {account}',stmtNoAccountInFile:'Die Datei nennt kein Konto.',stmtRowsFound:'{n} Buchungen gefunden',stmtBalanceLine:'Schlussstand laut Datei: {amount} zum {date}',stmtNoBalance:'Die Datei enthält keinen Kontostand.',stmtRowsHint:'Wähle ab, was nicht importiert werden soll.',stmtNoRows:'Diese Datei enthält keine Buchungen – nur einen Kontostand.',importStatement:'Kontoauszug importieren',stmtBalanceApplied:'Kontostand übernommen: {amount} zum {date}.',stmtBalanceSkippedNewerProvider:'Kontostand nicht übernommen: die Bank hat für diesen Tag oder später schon einen Stand gemeldet.',stmtBalanceSkippedNewerManual:'Kontostand nicht übernommen: für diesen Tag oder später gibt es schon einen von Hand erfassten Stand.',stmtBalanceSkippedCurrency:'Kontostand nicht übernommen: die Datei nennt eine andere Währung als das Konto.',remembered:'wie beim letzten Import',stmtAdapterIkano:'Ikano-Kontoauszug (PDF)',stmtAdapterC24:'C24-Kontoauszug (PDF)',stmtWarningsTitle:'Bitte prüfen',warn_not_reconciled:'Die gelesenen Buchungen führen nicht vom Anfangs- zum Endstand des Auszugs. Mindestens ein Betrag ist falsch gelesen, deshalb ist keine Zeile vorgewählt – prüfe jede einzeln gegen den Auszug.',warn_rows_not_read:'Die Buchungen dieses Auszugs kann FullWorth noch nicht lesen. Übernommen wird nur der Kontostand.',warn_credit_limit_mismatch:'Kreditrahmen minus verfügbarer Betrag ergibt nicht den Saldo des Auszugs. Prüfe den Kontostand.',note_not_reconciled:'nicht gesichert – gegen den Auszug prüfen',note_internal_transfer:'interne Umbuchung zwischen Karte und Ratenkauf – hebt sich im Konto auf'},
  en:{subtitle:'Import transactions and portfolios from other apps.',back:'Back',space:'FullWorth Space',txHint:'For bank and finance-app exports. CSV and XLSX are analysed first; you confirm columns and target accounts before committing.',invHint:'For Parqet, Finanzfluss and other portfolio exports. Buys, sells, dividends, interest, fees and taxes are validated.',analyse:'Analyse file',review:'Review import',import:'Import',importDepot:'Import portfolio',mapping:'Map columns',targets:'Map accounts',targetsHint:'Every source account found in the file is mapped to a FullWorth account.',securities:'Review securities',working:'Processing …',needFile:'Choose a CSV or XLSX file first.',needMapping:'Date and amount must be mapped.',needInvestmentMapping:'Trade date and transaction type must be mapped.',needAccount:'Map every source account to a target account.',needPortfolio:'Choose a target portfolio.',done:'Import completed.',error:'Import failed.',rows:'Rows',ready:'Ready',errors:'Errors',imported:'Imported',duplicates:'Duplicates',newPortfolio:'Create new import portfolio',none:'Do not map',autoNew:'Automatic / create new',matched:'Auto matched',willCreate:'Will be created',noAccounts:'No writable accounts found.',noPortfolios:'No portfolio exists yet.',detectTitle:'Import a file',detectHint:'Choose a file — FullWorth works out where it came from.',detectOr:'or pick the source yourself',detecting:'Reading the file …',detectContinue:'Continue',detectUnknown:'This file cannot be matched to a source with confidence. Please pick the source yourself.',detectFailed:'The file could not be read.',srcFinanzguru:'Finanzguru export',srcStatement:'Bank statement',srcTransactions:'Transactions (CSV/XLSX)',srcInvestments:'Portfolio (CSV/XLSX)',srcBrokerPdf:'PDF',srcUnknown:'Unknown',whyFinanzguru:'The workbook carries the Finanzguru export headers.',whyMt940:'The file is an MT940 statement.',whyCamt:'The file is a CAMT statement.',whyPdf:'The file is a PDF but not a bank statement FullWorth knows – it goes to the portfolio import for broker statements.',whyIkanoPdf:'The file is an Ikano Bank statement (IKEA credit card).',whyC24Pdf:'The file is a C24 Bank statement.',whyInvestments:'The table carries security columns.',whyTabular:'The table has a date and an amount column.',whyUnmapped:'The table has neither a recognisable date nor amount column.',whyUnreadable:'No reader understands this file.',whyNoRows:'The file has no rows beyond its header.',detectAccounts:'{n} accounts',detectRows:'{n} bookings',detectPeriod:'{from} – {to}',historyEmpty:'No portfolio imports yet.',rollback:'Roll back import',rollbackConfirm:'Really roll back this portfolio import? Later dependent trades automatically block an unsafe rollback.',rolledBack:'Import rolled back.',healthy:'Plausible',checkWarnings:'Warnings',cash:'Cash',positions:'Positions',rowsTitle:'Review bookings',rowsHint:'Uncheck anything you do not want to import. Rows with errors cannot be selected.',selectAll:'Select all',selectedOf:'{n} of {total} selected',errorsTitle:'Row cannot be imported',notImportable:'not importable',noRows:'No rows found.',showing:'Showing the first {n} of {total} rows. The selection applies to all of them.',noneSelected:'Select at least one booking.',duplicate:'Already there',dupInFile:'appears twice in this file',dupExternalKey:'booking id was already imported',dupExisting:'same date, amount and counterparty already booked',dupProbable:'Probably already there: {name} on {date}, from another source. Tick it if it is a second payment.',dupHint:'{n} bookings already or probably exist and were unchecked.',dupChecking:'Checking for duplicates …',txHistory:'Recent transaction imports',txHistoryEmpty:'No transaction imports yet.',txRollback:'Roll back import',txRollbackConfirm:'Really roll back this import? Bookings you have since split, tagged, reviewed or linked to a contract are kept.',txRolledBack:'{removed} removed, {kept} kept.',statusCompleted:'Completed',statusCancelled:'Cancelled',statusFailed:'Failed',statusRolledBack:'Rolled back',categoryTargets:'Map categories',categoryTargetsHint:'Every category found in the file gets a FullWorth category. "Automatic" takes a category of the same name or leaves it to the rules.',autoCategory:'Automatic',noCategoriesFound:'The file has no category column.',needStatementFile:'Choose an MT940 or CAMT file first.',needStatementAccount:'Choose an account.',newAccount:'Create a new account',newAccountName:'Name of the new account',newAccountDefault:'Imported account',needAccountName:'Enter a name for the new account.',stmtHint:'For accounts FullWorth cannot connect to directly: upload the MT940 or CAMT file, or the PDF statement from Ikano or C24, and choose the account it belongs to.',stmtAccountFromFile:'Account per file: {account}',stmtNoAccountInFile:'The file names no account.',stmtRowsFound:'{n} bookings found',stmtBalanceLine:'Closing balance per file: {amount} as of {date}',stmtNoBalance:'The file contains no balance.',stmtRowsHint:'Uncheck anything you do not want to import.',stmtNoRows:'This file has no bookings — only a balance.',importStatement:'Import statement',stmtBalanceApplied:'Balance applied: {amount} as of {date}.',stmtBalanceSkippedNewerProvider:'Balance not applied: the bank already reported a balance for this day or later.',stmtBalanceSkippedNewerManual:'Balance not applied: a manually entered balance already exists for this day or later.',stmtBalanceSkippedCurrency:'Balance not applied: the file states a different currency than the account.',remembered:'as in the last import',stmtAdapterIkano:'Ikano statement (PDF)',stmtAdapterC24:'C24 statement (PDF)',stmtWarningsTitle:'Please check',warn_not_reconciled:'The bookings read do not lead from the statement\'s opening to its closing balance. At least one amount was misread, so no row is preselected – check each one against the statement.',warn_rows_not_read:'FullWorth cannot read the bookings of this statement yet. Only the balance is imported.',warn_credit_limit_mismatch:'Credit limit minus available amount does not equal the statement balance. Check the balance.',note_not_reconciled:'not verified – check against the statement',note_internal_transfer:'internal transfer between card and instalments – cancels out within the account'}
}[lang];
...
setText('detect-title',t.detectTitle);setText('detect-hint',t.detectHint);setText('detect-or',t.detectOr);
setText('stmt-hint',t.stmtHint);setText('stmt-detect',t.analyse);setText('stmt-commit',t.importStatement);setText('stmt-target-hint',lang==='de'?'Wähle das Konto, zu dem der Auszug gehört — oder lege ein neues an.':'Choose the account this statement belongs to, or create a new one.');setText('stmt-rows-hint',t.stmtRowsHint);setText('stmt-no-rows',t.stmtNoRows);
...
// Was beim letzten Import dieser Bezeichnung bestaetigt wurde (#131, Abschnitt 3), steht vor dem
// geratenen Vorschlag: der Rat ueber IBAN-Endung und Namen ist eine Vermutung, die Erinnerung eine
// Entscheidung des Nutzers. Ohne sie musste dieselbe Zuordnung bei jedem Import neu getroffen werden.
function accountSelect(source,rememberedId=null){const select=document.createElement('select');select.className='account-map-select';const empty=document.createElement('option');empty.value='';empty.textContent=t.none;select.appendChild(empty);const fresh=document.createElement('option');fresh.value=NEW_ACCOUNT;fresh.textContent=t.newAccount;select.appendChild(fresh);const guess=(rememberedId&&state.accounts.find(a=>a.id===rememberedId))||guessAccount(source);for(const account of state.accounts){const option=document.createElement('option');option.value=account.id;option.textContent=accountLabel(account);if(guess?.id===account.id)option.selected=true;select.appendChild(option)}return select}
...
// Die Zeile, die eine Dublette erklaert. "probable" (#131, Abschnitt 6) ist keine Dublette, sondern ein
// Vorschlag: dieselbe Zahlung, anders benannt, aus einer anderen Quelle - die Zeile nennt deshalb die
// vorhandene Buchung, mit der sie verwechselt werden koennte, statt "Dublette" zu sagen.
const duplicateLine = (reason, existing) => reason === 'probable'
  ? fill(t.dupProbable, { name: existing?.counterparty || '—', date: formatDateOnly(existing?.bookingDate) })
  : `${t.duplicate} — ${duplicateReasonText(reason)}`;

const investmentDuplicateReasonText = reason => reason === 'in_file'
  ? (lang === 'de' ? 'Derselbe Handel kommt in dieser Datei doppelt vor.' : 'The same trade appears twice in this file.')
  : reason === 'external_key'
    ? (lang === 'de' ? 'Diese Transaktions-ID wurde im Zieldepot schon importiert.' : 'This transaction id was already imported into the target portfolio.')
    : reason === 'existing'
      ? (lang === 'de' ? 'Derselbe Handel ist im Zieldepot schon vorhanden.' : 'The same trade already exists in the target portfolio.')
      : t.duplicate;
...
function candidateRow(candidate) {
  ...
  sub.textContent = [candidate.bookingDate, candidate.categoryText,
    ...
    duplicateReason ? duplicateLine(duplicateReason, state.tx.probableOf?.get(candidate.id)) : null].filter(Boolean).join(' · ');
...
async function refreshDuplicatePreview() {
  ...
  try {
    ...
    state.tx.duplicates = new Map((preview.candidates || []).filter(item => item.status === 'duplicate' || item.reason === 'probable').map(item => [item.id, item.reason]));
    state.tx.probableOf = new Map((preview.candidates || []).filter(item => item.reason === 'probable').map(item => [item.id, item.probableOf]));
    status('tx', '');
  } catch (err) {
...
function statementAccountSelect(source) {
  ...
  // Ein Auszug kann zu einem Konto gehoeren, das es hier noch nicht gibt. Bisher musste man es
  // vorher von Hand anlegen - ein Zwischenschritt, der nichts entscheidet.
  const fresh = document.createElement('option');
  fresh.value = NEW_ACCOUNT;
  fresh.textContent = t.newAccount;
...
function renderStatementAccountField() {
  ...
  // Der Name steht nur da, wenn er gebraucht wird - ein leeres Feld neben einer Auswahl, die es nicht
  // betrifft, fragt nach etwas, das niemand beantworten soll.
  const nameField = document.createElement('label');
  nameField.className = 'field';
  nameField.hidden = select.value !== NEW_ACCOUNT;
  const nameSpan = document.createElement('span');
  nameSpan.textContent = t.newAccountName;
  const nameInput = document.createElement('input');
  nameInput.id = 'stmt-new-account-name';
  nameInput.type = 'text';
  nameInput.maxLength = 120;
  nameInput.value = state.stmt.upload?.statementAccount || t.newAccountDefault;
  nameField.append(nameSpan, nameInput);
  root.appendChild(nameField);
  select.addEventListener('change', () => {
    nameField.hidden = select.value !== NEW_ACCOUNT;
    loadStatementCandidates().catch(err => error('stmt', err));
  });
...
const STATEMENT_ADAPTER_LABEL = {
  mt940: () => 'MT940',
  camt: () => 'CAMT',
  ikano_pdf: () => t.stmtAdapterIkano,
  c24_pdf: () => t.stmtAdapterC24
};
...
function renderStatementFound(upload, candidateCount) {
  ...
  addRow(
    STATEMENT_ADAPTER_LABEL[upload.adapter]?.() || upload.adapter,
    upload.statementAccount ? fill(t.stmtAccountFromFile, { account: upload.statementAccount }) : t.stmtNoAccountInFile
  );
  // Ein PDF wird nachgerechnet (#131, Abschnitt 11). Was dabei auffiel, steht hier - nicht versteckt in
  // einer Zeile, sondern da, wo man die Datei als Ganzes ansieht.
  for (const warning of upload.warnings || [])
    addRow(t.stmtWarningsTitle, t['warn_' + warning] || warning);
...
function stmtCandidateRow(candidate) {
  ...
  box.type = 'checkbox';
  box.dataset.candidate = candidate.id;
  // Eine Zeile mit Pruefnotiz ist importierbar, aber nicht ungefragt: nicht gesichert, oder eine interne
  // Umbuchung, die sich im Konto aufhebt. Sie steht da, abgewaehlt, mit ihrem Grund.
  // Dasselbe fuer eine Zeile, die auf dem gewaehlten Konto schon oder vermutlich schon steht.
  box.checked = !candidate.reviewNote && !candidate.duplicate;
  if (candidate.duplicate) row.classList.add('ic-candidate-duplicate');
  ...
  sub.textContent = [
    ...
    candidate.description && candidate.description !== title.textContent ? candidate.description : null,
    candidate.reviewNote ? (t['note_' + candidate.reviewNote] || candidate.reviewNote) : null,
    candidate.duplicate ? duplicateLine(candidate.duplicate, candidate.duplicateOf) : null
  ].filter(Boolean).join(' · ');
  main.append(title, sub);
  label.append(box, main);
  const amount = document.createElement('span');
...
// Die Zeilen, geprueft gegen das gewaehlte Zielkonto (#131, Abschnitt 6): was dort schon oder vermutlich
// schon steht, kommt abgewaehlt und sagt, womit. Ein neues Konto hat nichts, wogegen sich pruefen liesse.
async function loadStatementCandidates() {
  const chosen = $('stmt-account-select')?.value;
  const target = chosen && chosen !== NEW_ACCOUNT ? `&accountId=${encodeURIComponent(chosen)}` : '';
  state.stmt.candidates = await api(`api/import-jobs/${state.stmt.jobId}/candidates?fullWorthSpaceId=${encodeURIComponent(state.space.id)}${target}`);
...
async function commitStatement() {
  try {
    const chosen = $('stmt-account-select')?.value;
    if (!chosen) throw new Error(t.needStatementAccount);
    const newName = chosen === NEW_ACCOUNT ? ($('stmt-new-account-name')?.value || '').trim() : '';
    if (chosen === NEW_ACCOUNT && !newName) throw new Error(t.needAccountName);
    status('stmt', t.working);
...
function syncPortfolioTarget(){const creating=$('inv-portfolio').value==='__new__';$('inv-new-portfolio-fields').hidden=!creating;$('inv-new-portfolio-name').required=creating;$('inv-new-portfolio-currency').required=creating;if(state.inv.jobId)void refreshInvestmentDuplicatePreview()}
...
// --- Schritt 0: welche Quelle ist das? (#131) ---------------------------------------------------
//
// Vorher waehlte der Nutzer erst eine von fuenf Kacheln und danach die Datei. Wer falsch waehlte,
// landete in einer Sackgasse, deren Fehlermeldung von der Datei handelte und nicht von der Wahl.
// Jetzt liest der Server die Datei einmal und sagt, welcher Weg sie verarbeiten kann.
//
// Zweimal hochgeladen wird dabei nichts: die Datei liegt noch im Browser, und jeder Zielweg - auch
// der auf einer "anderen Seite" - steht im selben Dokument, weil eine Seite hier eine Ansicht ist und
// kein eigenes Dokument. Uebergeben wird deshalb das File-Objekt selbst.
const SOURCE_LABEL={finanzguru:()=>t.srcFinanzguru,statement:()=>t.srcStatement,transactions:()=>t.srcTransactions,investments:()=>t.srcInvestments,'broker-pdf':()=>t.srcBrokerPdf,unknown:()=>t.srcUnknown};
const REASON_TEXT={finanzguruHeaders:()=>t.whyFinanzguru,mt940:()=>t.whyMt940,camt:()=>t.whyCamt,ikano_pdf:()=>t.whyIkanoPdf,c24_pdf:()=>t.whyC24Pdf,pdf:()=>t.whyPdf,investmentColumns:()=>t.whyInvestments,tabularColumns:()=>t.whyTabular,unmappedColumns:()=>t.whyUnmapped,unreadable:()=>t.whyUnreadable,noRows:()=>t.whyNoRows};

// Ein File-Input laesst sich nur ueber eine DataTransfer-Liste befuellen; eine einzelne Datei
// zuzuweisen ist nicht erlaubt. Genau dafuer gibt es sie.
function handOverFile(inputId,file){
  const input=$(inputId);
...
function continueWithDetected(detection,file){
  if(detection.adapter==='finanzguru'){handOverFile('finanzguru-file',file);navigate('import-finanzguru-xlsx');return}
  if(detection.adapter==='broker-pdf'){handOverFile('pdf-file',file);navigate('import-broker-pdf');return}
  const mode=detection.adapter==='statement'?'statement':detection.adapter==='investments'?'investments':'transactions';
  handOverFile(mode==='statement'?'stmt-file':mode==='investments'?'inv-file':'tx-file',file);
  activateMode(mode);
  $(modeArticles[mode])?.scrollIntoView({behavior:'smooth',block:'start'});
  // Der naechste Schritt ist auf allen drei Wegen derselbe Knopf. Er wird hier gleich gedrueckt: die
  // Datei ist gewaehlt, ein zweites "analysieren" waere dieselbe Entscheidung noch einmal.
  $(mode==='statement'?'stmt-detect':mode==='investments'?'inv-detect':'tx-detect')?.click();
}

function fmtDate(value){const date=new Date(String(value));return Number.isNaN(date.getTime())?String(value):date.toLocaleDateString(lang==='en'?'en-US':'de-DE')}

function renderDetection(detection,file){
  const box=$('detect-result');
  box.replaceChildren();
  box.hidden=false;
  const facts=[];
  if(detection.accounts?.length)facts.push(detection.accounts.length===1?detection.accounts[0]:t.detectAccounts.replace('{n}',String(detection.accounts.length)));
  if(detection.rows>0)facts.push(t.detectRows.replace('{n}',String(detection.rows)));
  if(detection.from&&detection.to)facts.push(t.detectPeriod.replace('{from}',fmtDate(detection.from)).replace('{to}',fmtDate(detection.to)));
  ...
  const title=document.createElement('div');
  title.className='row-title';
  title.textContent=(SOURCE_LABEL[detection.adapter]||SOURCE_LABEL.unknown)();
  const sub=document.createElement('div');
  sub.className='row-sub';
  sub.textContent=[(REASON_TEXT[detection.reasonKey]||(()=>''))(),...facts].filter(Boolean).join(' · ');
  main.append(title,sub);
  row.appendChild(main);

  if(detection.adapter==='unknown'){
    const hint=document.createElement('div');
    hint.className='row-sub';
    hint.textContent=t.detectUnknown;
    main.appendChild(hint);
  }else{
    const go=document.createElement('button');
    go.type='button';
    go.className=buttonClass(ButtonRole.Primary);
    go.textContent=t.detectContinue;
    go.addEventListener('click',()=>continueWithDetected(detection,file));
    row.appendChild(go);
  }
  box.appendChild(row);
}
...
async function detectSource(){
  ...
  if(!file)return;
  $('detect-result').hidden=true;
  status('detect',t.detecting);
  try{
    const detection=await api(`api/import/detect?fullWorthSpaceId=${encodeURIComponent(state.space.id)}`,{method:'POST',body:await formWithFile(file)});
    status('detect','');
    renderDetection(detection,file);
  }catch(err){console.error(err);status('detect',t.detectFailed);$('detect-status')?.classList.add('is-error')}
}
...
$('detect-file').addEventListener('change',detectSource);
...
// Die Knöpfe sind oben verdrahtet, einmal beim Laden des Moduls - sie stehen im Dokument und
// verschwinden nicht. Was bei jedem Besuch neu muss, ist nur das Laden.
export async function renderImportCenter() {
  await boot();
}
