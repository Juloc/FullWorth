...
async function showTransferCandidates() {
  ...
  hint.innerHTML = `<span class="tx-transfer-hint-text"><span aria-hidden="true">⟳</span> ${ctx.esc(label)}</span>` +
    `<button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-review>${ctx.esc(ctx.get('transactions.review'))}</button>`;
  hint.hidden = false;
...
async function openDetail(listItem) {
  ...
  const dlg = ctx.dialog(`<form class="dialog-card tx-detail" method="dialog">
    ...
    ${t.amount > 0 ? `<div class="tx-refund"><div class="row-main"><div class="row-title">${ctx.esc(ctx.get('transactions.refund'))}</div><div class="row-sub">${t.refundOfTransactionId ? ctx.esc(ctx.get(t.refundCategoryId ? 'transactions.refundLinkedItem' : 'transactions.refundLinked')) : ctx.esc(ctx.get('transactions.refundHint'))}</div></div><div class="row-side"><button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-refund-link>${ctx.esc(ctx.get('transactions.refundLink'))}</button>${t.refundOfTransactionId ? `<button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-refund-clear>${ctx.esc(ctx.get('transactions.refundClear'))}</button>` : ''}</div></div>` : ''}
    ...
    <!-- "Coach fragen" and "Teilen" are navigations, not decisions about this drawer, so they sit as
         rows next to the receipt link instead of crowding the actions row. Five buttons in one row is
         what the audit measured here (docs/UI_AUDIT.md step 4); three is a choice a person can make. -->
    <button type="button" class="row settings-link tx-detail-jump" data-split><div class="row-main"><div class="row-title">${ctx.esc(ctx.get('transactions.split'))}</div></div><span aria-hidden="true">›</span></button>
    <button type="button" class="row settings-link tx-detail-jump" data-coach><div class="row-main"><div class="row-title">${ctx.esc(deLabel('Coach fragen','Ask Coach'))}</div></div><span aria-hidden="true">›</span></button>
    <div class="dialog-actions">${t.isManual ? `<button type="button" class="btn btn-danger" data-delete>${ctx.esc(ctx.get('transactions.delete'))}</button><span class="fw-actions-spacer"></span>` : ''}<button type="button" class="btn btn-secondary" data-cancel>${ctx.esc(ctx.get('common.cancel'))}</button><button type="button" class="btn btn-primary" data-save>${ctx.esc(ctx.get('common.apply'))}</button></div>
  </form>`);
...
async function openSplitDialog(t) {
  ...
  const dlg = ctx.dialog(`<form class="dialog-card tx-split tx-split-articles" method="dialog">
    ...
    <button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-add-line>${ctx.esc(ctx.get('transactions.addLine'))}</button>
    <div class="split-remaining" data-remaining></div>
    <div class="dialog-actions"><button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-cancel>${ctx.esc(ctx.get('common.cancel'))}</button><button type="button" class="${buttonClass(ButtonRole.Primary)}" data-save>${ctx.esc(ctx.get('common.apply'))}</button></div>
  </form>`);
  dlg.classList.add('drawer');
  ...
  const rowFor = line => {
    ...
    row.innerHTML = `<select class="split-article" aria-label="${ctx.esc(articleLabel)}">
      ...
      <button type="button" class="${buttonClass(ButtonRole.Icon)}" data-remove aria-label="${ctx.esc(ctx.get('dashboard.remove'))}">×</button>`;
