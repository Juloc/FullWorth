...
export function mountExportAndWarrantyActions(panel, { api, esc, makeDialog, money, fmtDate, showError, bffUrl }) {
  ...
  actions.innerHTML = `<button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-pa-warranty>${esc(text('Fristen', 'Deadlines'))}</button>
    <button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-pa-export>${esc(text('Export', 'Export'))}</button>`;
  head.appendChild(actions);

  actions.querySelector('[data-pa-warranty]').onclick = async () => {
    const dlg = makeDialog(`<div class="pa-dialog-card pa-picker"><div class="panel-head"><h2>${esc(text('Garantie- & Rückgabefristen', 'Warranty & return deadlines'))}</h2><button type="button" data-close>×</button></div><div class="pa-toolbar"><select data-days><option value="30">30 ${esc(text('Tage', 'days'))}</option><option value="90" selected>90 ${esc(text('Tage', 'days'))}</option><option value="365">365 ${esc(text('Tage', 'days'))}</option></select><button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-load>${esc(text('Aktualisieren', 'Refresh'))}</button></div><div data-results class="pa-list"></div><div class="pa-dialog-error" data-error hidden></div></div>`);
...
function mountDocuments({ dlg, purchase, writable, api, esc, makeDialog, showError, refresh, confirmAction }) {
  ...
  for (const doc of purchase.documents || []) {
    ...
    actions.innerHTML = `<button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-document-runs>${esc(text('OCR-Verlauf', 'OCR history'))}</button>${writable ? `<button type="button" class="${buttonClass(ButtonRole.Secondary)}" data-document-extract>${esc(text('OCR neu', 'Run OCR'))}</button><button type="button" class="${buttonClass(ButtonRole.Danger)}" data-document-delete>${esc(text('Löschen', 'Delete'))}</button>` : ''}`;
