...
function renderHistoryList(){
  ...
  root.innerHTML=entries.map(e=>`
    <article class="panel history-entry" data-history-id="${e.id}">
      ...
      <div class="history-entry-main"><div><span class="history-event-badge">${eventLabel(e.eventType)}</span></div><h3>${esc(e.title)}</h3>${e.note?`<p>${esc(e.note)}</p>`:''}<div class="history-changes">${(e.changedFields||[]).slice(0,8).map(f=>`<span class="history-change">${esc(fieldLabel(f))}</span>`).join('')}${(e.changedFields||[]).length>8?`<span class="history-change">+${e.changedFields.length-8}</span>`:''}</div></div>
