...
async function renderOverview(state,container){
  ...
  ${canManage?`<div class="ip-actions"><button type="button" data-ip-add>${esc(text('Transaktion hinzufügen','Add transaction'))}</button><button type="button" class="btn btn-secondary" data-ip-price>${esc(text('Kurs erfassen','Add price'))}</button><button type="button" class="btn btn-secondary" data-ip-settings>${esc(text('Depot-Einstellungen','Portfolio settings'))}</button></div>`:''}`;
  if(canManage){
