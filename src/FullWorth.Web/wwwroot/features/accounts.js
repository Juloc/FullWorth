import { converted, maskIdentifier } from '../ui/money.js';
import { state } from '../core/state.js';
import {
  attachManualAccountVisualFields,
  initializeAccountPresentation,
...
const use=context=>{if(context)ctx=context;if(!ctx)throw new Error('Accounts context not initialized.');return ctx};
...
const money=(value,currency)=>ctx.money(value,currency);
const toast=(message,duration)=>ctx.toast(message,duration);
const dialog=(html,options)=>ctx.dialog(html,options);
const jsonBody=data=>ctx.jsonBody(data);
const empty=(element,message)=>ctx.empty(element,message);
const showView=(view,options)=>ctx.showView(view,options);
const loadCurrent=()=>ctx.reload();
const acctId=last4=>last4?` · ${maskIdentifier(last4)}`:'';

async function rerenderAccounts(){
  await renderAccountsInternal();
  invalidateAccountPresentation();
...
export async function renderAccounts(context){
  use(context);
  await rerenderAccounts();
}

export function newAccount(context=ctx){
  use(context);
  return openAddAccountDialogInternal();
}

export { refreshAccountPresentation };
...
async function renderAccountsInternal(){
...
function openAccountNameDialog(account){
  ...
  dlg.querySelector('[data-close]').onclick=()=>dlg.close();
  const readVisual=attachManualAccountVisualFields(dlg);
  ...
  dlg.querySelector('form').onsubmit=async e=>{
    ...
    try{
      ...
      dlg.close();toast(get('common.saved'));await rerenderAccounts();
...
async function deleteAccount(account){
  ...
  try{await api(`api/accounts/${account.id}`,{method:'DELETE'});toast(get('accounts.deleted'));await rerenderAccounts()}
  catch(err){toast(err.message||get('common.error'))}
}
...
async function deleteGroup(g){
  ...
  try{await api(`api/account-groups/${g.id}`,{method:'DELETE'});toast(get('accounts.groupDeleted'));await rerenderAccounts()}
  catch(err){toast(err.message||get('common.error'))}
}
...
function openMoveToGroupDialog(account,groups){
  ...
  dlg.querySelector('form').onsubmit=async e=>{
    ...
    try{await api(`api/accounts/${account.id}/group`,{...jsonBody({groupId}),method:'PUT'});dlg.close();await rerenderAccounts()}
    catch(err){toast(err.message||get('common.error'))}
  };
  dlg.showModal();
}
...
async function disconnectConnection(connection,button){
  ...
  form.onsubmit=async e=>{
    ...
    try{
      ...
      dlg.close();toast(get(deleteLocalData?'accounts.disconnected':'accounts.disconnectedKept'));await rerenderAccounts();
...
function openAddAccountDialogInternal(){
...
function openManualAccountDialog(){
  ...
  dlg.querySelector('form').onsubmit=async e=>{
    ...
    try{
      ...
      await persistAccountVisual(created?.id,readVisual());
      dlg.close();toast(get('accounts.created'));await rerenderAccounts();
    }catch(err){toast(err.message||get('common.error'))}
  };
  dlg.showModal();
}
function openBalanceDialog(account){
  ...
  dlg.querySelector('form').onsubmit=async e=>{
    e.preventDefault();const fd=new FormData(e.currentTarget);
    try{await api(`api/accounts/${account.id}/balance`,{...jsonBody({amount:Number(fd.get('amount')),currency:null}),method:'PUT'});dlg.close();toast(get('accounts.balanceUpdated'));await rerenderAccounts()}catch(err){toast(err.message||get('common.error'))}
  };
  dlg.showModal();
}
...
export async function renderEnableBankingSettings(context=ctx){use(context);
...
export function openEnableBankingWizard(initialStatus,options={},context=ctx){use(context);
