...
export function mountExportAndWarrantyActions(panel, { api, esc, makeDialog, money, fmtDate, showError }) {
  ...
  actions.innerHTML = `<button type="button" class="btn btn-secondary" data-pa-warranty>${esc(text('Fristen', 'Deadlines'))}</button>
    <button type="button" class="btn btn-secondary" data-pa-export>${esc(text('Export', 'Export'))}</button>`;
...
async function openReturnsDialog({ parent, purchase, item, api, esc, makeDialog, money, fmtDate, showError, refresh }) {
  ...
  const dlg = makeDialog(`<form class="pa-dialog-card pa-picker"><div class="panel-head"><div><h2>${esc(text('Retoure / Erstattung', 'Return / refund'))}</h2><div class="row-sub">${esc(item.name)} · ${esc(text('Noch verfügbar', 'Remaining'))}: ${remaining}</div></div><button type="button" data-close>×</button></div>
    <div class="pa-list" data-return-list>${(returns || []).map(row => `<div class="pa-history-row"><div><strong>${row.quantity}× · ${esc(money(row.amount, row.currency))}</strong><span>${esc(fmtDate(row.createdAt))}${row.note ? ` · ${esc(row.note)}` : ''}</span></div><button type="button" class="btn btn-danger" data-delete-return="${row.id}">${esc(text('Entfernen', 'Remove'))}</button></div>`).join('') || `<div class="state-empty">${esc(text('Noch keine Retoure.', 'No return recorded yet.'))}</div>`}</div>
...
function mountDocuments({ dlg, purchase, writable, api, esc, makeDialog, showError, refresh }) {
  ...
  for (const doc of purchase.documents || []) {
    ...
    actions.innerHTML = `<button type="button" class="btn btn-secondary" data-document-runs>${esc(text('OCR-Verlauf', 'OCR history'))}</button>${writable ? `<button type="button" class="btn btn-secondary" data-document-extract>${esc(text('OCR neu', 'Run OCR'))}</button><button type="button" class="btn btn-danger" data-document-delete>${esc(text('Löschen', 'Delete'))}</button>` : ''}`;
