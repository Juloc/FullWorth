...
function paperlessPane() {
  return `<div class="receipt-import-card">
    <div class="panel-head"><div><h3>Paperless-ngx</h3><div class="row-sub" data-paperless-state>${esc(t('Verbindung wird geprüft …', 'Checking connection …'))}</div></div><button type="button" class="ghost" data-paperless-test>${esc(t('Testen', 'Test'))}</button></div>
    <div class="receipt-import-grid">
      ...
      <label class="wide"><span>${esc(t('Standard-Suche', 'Default search'))}</span><input data-paperless-query type="text" placeholder="type:receipt"></label>
    </div>
    <div class="receipt-import-actions"><button type="button" class="ghost" data-paperless-save>${esc(t('Verbindung speichern', 'Save connection'))}</button><button type="button" class="danger ghost" data-paperless-delete>${esc(t('Trennen', 'Disconnect'))}</button></div>
  </div>
  <div class="receipt-import-card">
    <h3>${esc(t('Dokumente auswählen', 'Select documents'))}</h3>
    <div class="receipt-import-grid"><label class="wide"><span>${esc(t('Paperless-Suche für diesen Import', 'Paperless search for this import'))}</span><input data-paperless-filter type="text"></label></div>
    <div class="receipt-import-actions"><button type="button" class="ghost" data-paperless-preview>${esc(t('Vorschau', 'Preview'))}</button><label class="check inline"><input data-paperless-auto type="checkbox" checked> ${esc(t('Direkt analysieren', 'Analyze immediately'))}</label><button type="button" data-paperless-import>${esc(t('Auswahl importieren', 'Import selection'))}</button></div>
...
async function refreshPaperlessConnection() {
  ...
  try {
    ...
    if (!connection?.configured) { state.textContent = t('Nicht verbunden', 'Not connected'); return; }
    dialog.querySelector('[data-paperless-url]').value = connection.baseUrl || '';
    dialog.querySelector('[data-paperless-query]').value = connection.defaultQuery || '';
    dialog.querySelector('[data-paperless-filter]').placeholder = connection.defaultQuery || t('optional', 'optional');
...
async function savePaperlessConnection() {
  ...
  const defaultQuery = dialog.querySelector('[data-paperless-query]').value.trim() || null;
  if (!url || !token) return setBox('[data-paperless-preview-result]', t('URL und API-Token sind erforderlich.', 'URL and API token are required.'), 'error');
  setBusy('[data-paperless-save]', true);
  try {
    const result = await api('api/purchases/receipt-imports/paperless/connection', {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ baseUrl: url, apiToken: token, defaultQuery, isEnabled: true })
    });
  ...
  } catch (error) { setBox('[data-paperless-preview-result]', error.message, 'error'); }
  finally { setBusy('[data-paperless-save]', false); }
}
...
async function testPaperless() {
  ...
  } catch (error) { dialog.querySelector('[data-paperless-state]').textContent = error.message; }
  finally { setBusy('[data-paperless-test]', false); }
}

async function previewPaperless() {
  setBusy('[data-paperless-preview]', true);
  try {
    const preview = await api('api/purchases/receipt-imports/paperless/preview', json({ query: paperlessFilter(), limit: 500 }));
    renderPaperlessPreview(preview);
  } catch (error) { setBox('[data-paperless-preview-result]', error.message, 'error'); }
  finally { setBusy('[data-paperless-preview]', false); }
}
...
function renderPaperlessPreview(preview) {
  ...
  if (!docs.length) { el.innerHTML = `<div class="row-sub">${esc(t('Keine Dokumente gefunden.', 'No documents found.'))}</div>`; return; }
  el.innerHTML = `<div class="receipt-import-preview-head"><label class="check inline"><input type="checkbox" data-paperless-all checked> ${esc(t('Alle auswählen', 'Select all'))}</label><span>${preview.count} ${esc(t('gefunden', 'found'))}${preview.truncated ? ` · ${esc(t('Vorschau begrenzt', 'preview limited'))}` : ''}</span></div>
    <div class="receipt-import-docs">${docs.map(doc => `<label class="receipt-import-doc"><input type="checkbox" data-paperless-doc value="${doc.id}" checked><span><strong>${esc(doc.title || `#${doc.id}`)}</strong><small>${esc(doc.created || '')}</small></span></label>`).join('')}</div>`;
...
async function importPaperless() {
  ...
  try {
    const batch = await api('api/purchases/receipt-imports/paperless/import', json({
      filter: { query: paperlessFilter(), limit: 500 }, documentIds: selected, currency: currentCurrency(), autoStart: dialog.querySelector('[data-paperless-auto]').checked
    }));
    renderBatchResult('[data-paperless-preview-result]', batch); await refreshBatches();
  } catch (error) { setBox('[data-paperless-preview-result]', error.message, 'error'); }
  finally { setBusy('[data-paperless-import]', false); }
}
...
function paperlessFilter() { return dialog.querySelector('[data-paperless-filter]').value.trim() || null; }
