...
function render(packs) {
  ...
  list.querySelectorAll('[data-pack-id]').forEach(row => {
    ...
    row.querySelector('[data-pack-delete]')?.addEventListener('click', async () => {
      if (!await confirmMessage({title:'Brand-Pack löschen',message:'Dieses eigene Brand-Pack löschen? Das offizielle FullWorth-Pack bleibt unverändert.',confirmLabel:'Löschen',cancelLabel:'Abbrechen',destructive:true})) return;
      try {
