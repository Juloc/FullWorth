...
public static class BudgetScopeParityEndpoints
{
    ...
    {
        var uid=currentUser.RequireUserId(); if(!await RawSql.IsMemberAsync(db,uid,fullWorthSpaceId,ct)||!await db.Budgets.AsNoTracking().AnyAsync(x=>x.Id==budgetId&&x.FullWorthSpaceId==fullWorthSpaceId,ct))return Results.NotFound();
        var visible=await RawSql.VisibleAccountIdsAsync(db,uid,fullWorthSpaceId,ct);
    ...
    {
        var uid=currentUser.RequireUserId(); if(!await SpaceCapabilities.HasCapabilityAsync(db,uid,fullWorthSpaceId,"budgets.manage",ct))return Results.StatusCode(403);
        ...
        var visible=await RawSql.VisibleAccountIdsAsync(db,uid,fullWorthSpaceId,ct);if(accounts.Any(x=>!visible.Contains(x)))return Results.BadRequest(new{error="Budget account scope contains an inaccessible account."});
        ...
        await using var tx=await db.Database.BeginTransactionAsync(ct);var connection=await RawSql.OpenAsync(db,ct);
        foreach(var table in new[]{"BudgetCategories","BudgetAccounts","BudgetTags","BudgetMerchants"}){await using var del=RawSql.Command(connection,$"DELETE FROM \"{table}\" WHERE \"BudgetId\"=@b",("@b",budgetId));await del.ExecuteNonQueryAsync(ct);}
        foreach(var c in categories){await using var cmd=RawSql.Command(connection,"INSERT INTO \"BudgetCategories\" (\"BudgetId\",\"CategoryId\",\"IncludeDescendants\") VALUES (@b,@c,@d)",("@b",budgetId),("@c",c.CategoryId),("@d",c.IncludeDescendants));await cmd.ExecuteNonQueryAsync(ct);}
        foreach(var a in accounts){await using var cmd=RawSql.Command(connection,"INSERT INTO \"BudgetAccounts\" (\"BudgetId\",\"AccountId\") VALUES (@b,@a)",("@b",budgetId),("@a",a));await cmd.ExecuteNonQueryAsync(ct);}
        foreach(var tag in tags){await using var cmd=RawSql.Command(connection,"INSERT INTO \"BudgetTags\" (\"BudgetId\",\"TagId\") VALUES (@b,@t)",("@b",budgetId),("@t",tag));await cmd.ExecuteNonQueryAsync(ct);}
        foreach(var merchant in (request.Merchants??[]).Select(MerchantNormalization.Normalize).Where(x=>!string.IsNullOrWhiteSpace(x)).Distinct(StringComparer.OrdinalIgnoreCase)){await using var cmd=RawSql.Command(connection,"INSERT INTO \"BudgetMerchants\" (\"BudgetId\",\"NormalizedMerchant\") VALUES (@b,@m)",("@b",budgetId),("@m",merchant));await cmd.ExecuteNonQueryAsync(ct);}
        await using(var adv=RawSql.Command(connection,"""
...
""",("@b",budgetId),("@income",request.IncomeScheduleId),("@near",request.AlertNearPercent),("@critical",request.AlertCriticalPercent),("@group",request.GroupId),("@now",DateTimeOffset.UtcNow)))await adv.ExecuteNonQueryAsync(ct);
    ...
    {
        var uid=currentUser.RequireUserId();if(!await RawSql.IsMemberAsync(db,uid,fullWorthSpaceId,ct))return Results.NotFound();var budget=await db.Budgets.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==budgetId&&x.FullWorthSpaceId==fullWorthSpaceId,ct);if(budget is null)return Results.NotFound();
        var scope=await LoadScope(db,budgetId,ct);var visible=await RawSql.VisibleAccountIdsAsync(db,uid,fullWorthSpaceId,ct);var visibleScope=await RedactScopeAsync(db,scope,visible,ct);var day=asOf??DateOnly.FromDateTime(DateTime.UtcNow);var(windowStart,windowEnd)=await ResolveWindow(db,budget.Id,budget.Period,budget.StartDate,budget.EndDate,day,ct);
    ...
    {
        var c=await RawSql.OpenAsync(db,ct);var cats=new List<CategoryScopeWrite>();var accounts=new List<Guid>();var tags=new List<Guid>();var merchants=new List<string>();
        await using(var cmd=RawSql.Command(c,"SELECT \"CategoryId\",\"IncludeDescendants\" FROM \"BudgetCategories\" WHERE \"BudgetId\"=@b",("@b",budgetId))){await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))cats.Add(new(RawSql.Guid(r,"CategoryId"),RawSql.Bool(r,"IncludeDescendants")));}
        await using(var cmd=RawSql.Command(c,"SELECT \"AccountId\" FROM \"BudgetAccounts\" WHERE \"BudgetId\"=@b",("@b",budgetId))){await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))accounts.Add(RawSql.Guid(r,"AccountId"));}
        await using(var cmd=RawSql.Command(c,"SELECT \"TagId\" FROM \"BudgetTags\" WHERE \"BudgetId\"=@b",("@b",budgetId))){await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))tags.Add(RawSql.Guid(r,"TagId"));}
        await using(var cmd=RawSql.Command(c,"SELECT \"NormalizedMerchant\" FROM \"BudgetMerchants\" WHERE \"BudgetId\"=@b",("@b",budgetId))){await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))merchants.Add(RawSql.String(r,"NormalizedMerchant"));}
        Guid? income=null,group=null;decimal near=80,critical=100;await using(var cmd=RawSql.Command(c,"SELECT \"IncomeScheduleId\",\"AlertNearPercent\",\"AlertCriticalPercent\",\"GroupId\" FROM \"BudgetAdvancedSettings\" WHERE \"BudgetId\"=@b",("@b",budgetId))){await using var r=await cmd.ExecuteReaderAsync(ct);if(await r.ReadAsync(ct)){income=RawSql.NullableGuid(r,"IncomeScheduleId");near=RawSql.Decimal(r,"AlertNearPercent");critical=RawSql.Decimal(r,"AlertCriticalPercent");group=RawSql.NullableGuid(r,"GroupId");}}
    ...
    private static async Task<bool> CanReadIncomeScheduleAsync(FullWorthDbContext db,Guid space,Guid id,HashSet<Guid> visible,CancellationToken ct){var c=await RawSql.OpenAsync(db,ct);var sql=space==Guid.Empty?"SELECT \"AccountId\" FROM \"IncomeSchedules\" WHERE \"Id\"=@id":"SELECT \"AccountId\" FROM \"IncomeSchedules\" WHERE \"Id\"=@id AND \"FullWorthSpaceId\"=@space";await using var cmd=space==Guid.Empty?RawSql.Command(c,sql,("@id",id)):RawSql.Command(c,sql,("@id",id),("@space",space));var value=await cmd.ExecuteScalarAsync(ct);if(value is null)return false;return value is DBNull||visible.Contains((Guid)value);}
    ...
    private static async Task<bool> RawExists(FullWorthDbContext db,string sql,CancellationToken ct,params(string Name,object? Value)[] p){var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,sql,p);return await cmd.ExecuteScalarAsync(ct) is not null;}
    ...
    private static async Task<Dictionary<Guid,HashSet<Guid>>> LoadTagsByTransaction(FullWorthDbContext db,Guid[] ids,CancellationToken ct){var map=new Dictionary<Guid,HashSet<Guid>>();if(ids.Length==0)return map;var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"SELECT \"TransactionId\",\"TagId\" FROM \"TransactionTags\" WHERE \"TransactionId\"=ANY(@ids)",( "@ids",ids));await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var tx=RawSql.Guid(r,"TransactionId");if(!map.TryGetValue(tx,out var set))map[tx]=set=[];set.Add(RawSql.Guid(r,"TagId"));}return map;}
    private static async Task<(DateOnly Start,DateOnly End)> ResolveWindow(FullWorthDbContext db,Guid budgetId,string period,DateOnly? start,DateOnly? end,DateOnly asOf,CancellationToken ct){var p=(period??"monthly").ToLowerInvariant();if(p.Contains("salary")){var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"SELECT s.\"NextExpectedDate\",s.\"Cycle\",s.\"Interval\" FROM \"BudgetAdvancedSettings\" a JOIN \"IncomeSchedules\" s ON s.\"Id\"=a.\"IncomeScheduleId\" WHERE a.\"BudgetId\"=@b",("@b",budgetId));await using var r=await cmd.ExecuteReaderAsync(ct);if(await r.ReadAsync(ct)){var next=RawSql.NullableDate(r,"NextExpectedDate");if(next.HasValue){var prev=Previous(next.Value,RawSql.String(r,"Cycle"),RawSql.Int(r,"Interval"));while(next<=asOf){var old=next.Value;next=Next(old,RawSql.String(r,"Cycle"),RawSql.Int(r,"Interval"));prev=old;}return(prev,next.Value.AddDays(-1));}}}if(p.Contains("week")){var delta=((int)asOf.DayOfWeek+6)%7;var s=asOf.AddDays(-delta);return(s,s.AddDays(6));}if(p.Contains("quarter")){var m=((asOf.Month-1)/3)*3+1;var s=new DateOnly(asOf.Year,m,1);return(s,s.AddMonths(3).AddDays(-1));}if(p.Contains("year")){var s=new DateOnly(asOf.Year,1,1);return(s,new DateOnly(asOf.Year,12,31));}if(p.Contains("custom")&&start.HasValue)return(start.Value,end??start.Value.AddMonths(1).AddDays(-1));var ms=new DateOnly(asOf.Year,asOf.Month,1);return(ms,ms.AddMonths(1).AddDays(-1));}
