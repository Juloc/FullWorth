...
public sealed class FullWorthCloudClient : IFullWorthCloudClient
{
        ...
        CancellationToken ct)
    {
        if (events.Count is < 1 or > MaximumBatchEvents)
            throw new ArgumentOutOfRangeException(nameof(events), $"Cloud batch must contain 1-{MaximumBatchEvents} events.");
        ...
        var perEvent = result.Events?.Select(x => new FullWorthCloudBatchEventResult(
            ...
            .ToList() ?? [];
        return new(result.BatchId ?? batchId, result.Accepted, result.Duplicate, result.Rejected, perEvent);
    }
