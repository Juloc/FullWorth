...
public static class AnalyticsParityEndpoints
{
    ...
    private static async Task<Dictionary<Guid,HashSet<Guid>>> LoadContractMap(FullWorthDbContext db,Guid[] txIds,CancellationToken ct){var map=new Dictionary<Guid,HashSet<Guid>>();if(txIds.Length==0)return map;var c=await ParitySql.OpenAsync(db,ct);await using var cmd=ParitySql.Command(c,"""
...
""",( "@ids",txIds));await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var id=ParitySql.Guid(r,"TransactionId");if(!map.TryGetValue(id,out var set))map[id]=set=[];set.Add(ParitySql.Guid(r,"ContractId"));}return map;}
