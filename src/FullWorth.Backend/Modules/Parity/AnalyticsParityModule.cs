...
public static class AnalyticsParityEndpoints
{
    ...
    private static async Task<IResult> ListSaved(Guid fullWorthSpaceId,CurrentUserContext currentUser,FullWorthDbContext db,CancellationToken ct){var uid=currentUser.RequireUserId();if(!await RawSql.IsMemberAsync(db,uid,fullWorthSpaceId,ct))return Results.NotFound();var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"SELECT \"Id\",\"Name\",\"SchemaVersion\",\"ConfigJson\",\"CreatedAt\",\"UpdatedAt\" FROM \"SavedAnalyses\" WHERE \"FullWorthSpaceId\"=@space AND \"OwnerUserId\"=@uid ORDER BY \"Name\"",("@space",fullWorthSpaceId),("@uid",uid));await using var r=await cmd.ExecuteReaderAsync(ct);var rows=new List<object>();while(await r.ReadAsync(ct)){var json=RawSql.String(r,"ConfigJson");rows.Add(new{id=RawSql.Guid(r,"Id"),name=RawSql.String(r,"Name"),schemaVersion=RawSql.Int(r,"SchemaVersion"),config=JsonSerializer.Deserialize<JsonElement>(json),createdAt=RawSql.Timestamp(r,"CreatedAt"),updatedAt=RawSql.Timestamp(r,"UpdatedAt")});}return Results.Ok(rows);}
    private static async Task<IResult> CreateSaved(Guid fullWorthSpaceId,SavedAnalysisWrite request,CurrentUserContext currentUser,FullWorthDbContext db,AuditService audit,CancellationToken ct){var uid=currentUser.RequireUserId();if(!await RawSql.IsMemberAsync(db,uid,fullWorthSpaceId,ct))return Results.NotFound();if(string.IsNullOrWhiteSpace(request.Name)||ValidateQuery(request.Query) is not null)return Results.BadRequest(new{error="Invalid saved analysis."});var id=Guid.NewGuid();await WriteSaved(db,id,fullWorthSpaceId,uid,request,false,ct);audit.Record(fullWorthSpaceId,uid,"analysis.saved","SavedAnalysis",id);await db.SaveChangesAsync(ct);return Results.Ok(new{id});}
    ...
    private static async Task<IResult> DeleteSaved(Guid id,Guid fullWorthSpaceId,CurrentUserContext currentUser,FullWorthDbContext db,AuditService audit,CancellationToken ct){var uid=currentUser.RequireUserId();var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"DELETE FROM \"SavedAnalyses\" WHERE \"Id\"=@id AND \"FullWorthSpaceId\"=@space AND \"OwnerUserId\"=@uid",("@id",id),("@space",fullWorthSpaceId),("@uid",uid));if(await cmd.ExecuteNonQueryAsync(ct)==0)return Results.NotFound();audit.Record(fullWorthSpaceId,uid,"analysis.deleted","SavedAnalysis",id);await db.SaveChangesAsync(ct);return Results.NoContent();}
    ...
    private static async Task<Dictionary<Guid,HashSet<Guid>>> LoadTagMap(FullWorthDbContext db,Guid[] txIds,CancellationToken ct){var map=new Dictionary<Guid,HashSet<Guid>>();if(txIds.Length==0)return map;var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"SELECT \"TransactionId\",\"TagId\" FROM \"TransactionTags\" WHERE \"TransactionId\"=ANY(@ids)",( "@ids",txIds));await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var id=RawSql.Guid(r,"TransactionId");if(!map.TryGetValue(id,out var set))map[id]=set=[];set.Add(RawSql.Guid(r,"TagId"));}return map;}
    private static async Task<Dictionary<Guid,HashSet<Guid>>> LoadContractMap(FullWorthDbContext db,Guid[] txIds,CancellationToken ct){var map=new Dictionary<Guid,HashSet<Guid>>();if(txIds.Length==0)return map;var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"""
...
""",( "@ids",txIds));await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var id=RawSql.Guid(r,"TransactionId");if(!map.TryGetValue(id,out var set))map[id]=set=[];set.Add(RawSql.Guid(r,"ContractId"));}return map;}
    private static async Task<Dictionary<Guid,string>> LoadTagNames(FullWorthDbContext db,Guid space,CancellationToken ct){var map=new Dictionary<Guid,string>();var c=await RawSql.OpenAsync(db,ct);await using var cmd=RawSql.Command(c,"SELECT \"Id\",\"Name\" FROM \"FinanceTags\" WHERE \"FullWorthSpaceId\"=@space",("@space",space));await using var r=await cmd.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct))map[RawSql.Guid(r,"Id")]=RawSql.String(r,"Name");return map;}
    private static async Task<bool> WriteSaved(FullWorthDbContext db,Guid id,Guid space,Guid uid,SavedAnalysisWrite request,bool update,CancellationToken ct){var json=JsonSerializer.Serialize(new{query=request.Query,chartType=request.ChartType});var c=await RawSql.OpenAsync(db,ct);await using var cmd=update?RawSql.Command(c,"UPDATE \"SavedAnalyses\" SET \"Name\"=@name,\"SchemaVersion\"=@version,\"ConfigJson\"=CAST(@json AS jsonb),\"UpdatedAt\"=@now WHERE \"Id\"=@id AND \"FullWorthSpaceId\"=@space AND \"OwnerUserId\"=@uid",("@name",request.Name.Trim()),("@version",request.SchemaVersion),("@json",json),("@now",DateTimeOffset.UtcNow),("@id",id),("@space",space),("@uid",uid)):RawSql.Command(c,"INSERT INTO \"SavedAnalyses\" (\"Id\",\"FullWorthSpaceId\",\"OwnerUserId\",\"Name\",\"SchemaVersion\",\"ConfigJson\",\"CreatedAt\",\"UpdatedAt\") VALUES (@id,@space,@uid,@name,@version,CAST(@json AS jsonb),@now,@now)",("@id",id),("@space",space),("@uid",uid),("@name",request.Name.Trim()),("@version",request.SchemaVersion),("@json",json),("@now",DateTimeOffset.UtcNow));return await cmd.ExecuteNonQueryAsync(ct)>0;}
}
