...
""",("@id",c.Id),("@job",jobId),("@account",c.SourceAccount),("@date",c.Date),("@amount",c.Amount),("@currency",c.Currency),("@party",c.Counterparty),("@description",c.Description),("@category",c.Category),("@external",c.ExternalKey),("@fingerprint",c.Fingerprint),("@status",c.Status),("@error",c.Error));await cmd.ExecuteNonQueryAsync(ct);}audit.Record(fullWorthSpaceId,uid,"import.uploaded","ImportJob",jobId);await db.SaveChangesAsync(ct);await tx.CommitAsync(ct);return Results.Ok(new{jobId,fileName=file.FileName,adapter=ext==".csv"?"generic_csv":"generic_xlsx",sourceRows=candidates.Count,ready=candidates.Count-errors,errors,mapping});
    ...
    private static async Task<IResult> Rollback(Guid id,Guid fullWorthSpaceId,CurrentUserContext currentUser,FullWorthDbContext db,AuditService audit,CancellationToken ct)
    {
        ...
        if(!await PermissionsErgonomicsParityEndpoints.HasCapabilityAsync(db,uid,fullWorthSpaceId,"transactions.write",ct))return Results.StatusCode(403);
        var connection=await ParitySql.OpenAsync(db,ct);
        string status;DateTimeOffset? rolledBackAt;
        await using(var read=ParitySql.Command(connection,"SELECT \"Status\",\"RolledBackAt\" FROM \"ImportJobs\" WHERE \"Id\"=@id",("@id",id)))
        await using(var reader=await read.ExecuteReaderAsync(ct))
        {
            if(!await reader.ReadAsync(ct))return Results.NotFound();
            status=ParitySql.String(reader,"Status");rolledBackAt=ParitySql.NullableTimestamp(reader,"RolledBackAt");
        }
        ...
        int removed;
        await using(var delete=ParitySql.Command(connection,ImportTransactionProvenance.DeleteImportedTransactionsSql,("@job",id),("@space",fullWorthSpaceId)))
            removed=await delete.ExecuteNonQueryAsync(ct);
        var now=DateTimeOffset.UtcNow;
        await using(var candidates=ParitySql.Command(connection,"UPDATE \"ImportCandidates\" SET \"DuplicateStatus\"='rolled_back' WHERE \"ImportJobId\"=@job AND \"DuplicateStatus\"='imported'",("@job",id)))
            await candidates.ExecuteNonQueryAsync(ct);
        await using(var job=ParitySql.Command(connection,"UPDATE \"ImportJobs\" SET \"Status\"='rolled_back',\"RolledBackAt\"=@now,\"UpdatedAt\"=@now WHERE \"Id\"=@id",("@id",id),("@now",now)))
    ...
    private static string NormalizeCurrency(string? v){var s=Clean(v)?.ToUpperInvariant();return s is{Length:3}?s:"EUR";}
    ...
    private const string JobColumns="\"Id\",\"FileName\",\"AdapterKey\",\"Status\",\"SourceRowCount\",\"ReadyCount\",\"DuplicateCount\",\"ImportedCount\",\"ErrorCount\",\"CreatedAt\",\"CompletedAt\",\"RolledBackAt\",(SELECT count(*) FROM \"ImportTransactionLinks\" l WHERE l.\"ImportJobId\"=j.\"Id\")::int AS \"LinkCount\"";
    private static object JobRow(System.Data.Common.DbDataReader r)=>new{id=ParitySql.Guid(r,"Id"),fileName=ParitySql.String(r,"FileName"),adapterKey=ParitySql.String(r,"AdapterKey"),status=ParitySql.String(r,"Status"),sourceRowCount=ParitySql.Int(r,"SourceRowCount"),readyCount=ParitySql.Int(r,"ReadyCount"),duplicateCount=ParitySql.Int(r,"DuplicateCount"),importedCount=ParitySql.Int(r,"ImportedCount"),errorCount=ParitySql.Int(r,"ErrorCount"),createdAt=ParitySql.Timestamp(r,"CreatedAt"),completedAt=ParitySql.NullableTimestamp(r,"CompletedAt"),rolledBackAt=ParitySql.NullableTimestamp(r,"RolledBackAt"),
