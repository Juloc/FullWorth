...
""", ("@space", fullWorthSpaceId));
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            rows.Add(new
            {
                id = RawSql.Guid(reader,"Id"), name = RawSql.String(reader,"Name"), accountId,
                normalizedCounterparty = RawSql.NullableString(reader,"NormalizedCounterparty"), expectedAmount = RawSql.NullableDecimal(reader,"ExpectedAmount"),
                currency = RawSql.String(reader,"Currency"), cycle = RawSql.String(reader,"Cycle"), interval = RawSql.Int(reader,"Interval"),
                anchorDate = RawSql.NullableDate(reader,"AnchorDate"), nextExpectedDate = RawSql.NullableDate(reader,"NextExpectedDate"),
                valueMode = RawSql.String(reader,"ValueMode"), autoDetected = RawSql.Bool(reader,"AutoDetected"), isActive = RawSql.Bool(reader,"IsActive")
            });
        }
    ...
    {
        var userId = currentUser.RequireUserId();
        if (!await SpaceCapabilities.HasCapabilityAsync(db,userId,fullWorthSpaceId,"budgets.manage",ct)) return Results.StatusCode(403);
...
""",("@name",request.Name.Trim()),("@account",request.AccountId),("@party",MerchantNormalization.Normalize(request.NormalizedCounterparty)),("@amount",request.ExpectedAmount),("@currency",request.Currency.Trim().ToUpperInvariant()),("@cycle",NormalizeCycle(request.Cycle)),("@interval",Math.Max(1,request.Interval)),("@anchor",request.AnchorDate),("@next",request.NextExpectedDate),("@mode",NormalizeMode(request.ValueMode)),("@active",request.IsActive),("@now",DateTimeOffset.UtcNow),("@id",id),("@space",fullWorthSpaceId));
    ...
    private static async Task<IResult> ArchiveSchedule(Guid id, Guid fullWorthSpaceId, CurrentUserContext currentUser, FullWorthDbContext db, AuditService audit, CancellationToken ct)
    {
        ...
        if(!await CanWriteScheduleAsync(db,userId,fullWorthSpaceId,id,ct)) return Results.NotFound();
        var connection=await RawSql.OpenAsync(db,ct);
        await using var cmd=RawSql.Command(connection,"UPDATE \"IncomeSchedules\" SET \"IsActive\"=false,\"UpdatedAt\"=@now WHERE \"Id\"=@id AND \"FullWorthSpaceId\"=@space",("@now",DateTimeOffset.UtcNow),("@id",id),("@space",fullWorthSpaceId));
        if(await cmd.ExecuteNonQueryAsync(ct)==0) return Results.NotFound();
    ...
    private static async Task<IResult> DetectIncome(Guid fullWorthSpaceId, CurrentUserContext currentUser, FullWorthDbContext db, CancellationToken ct)
    {
        var userId=currentUser.RequireUserId();
        if(!await RawSql.IsMemberAsync(db,userId,fullWorthSpaceId,ct)) return Results.NotFound();
        var visible=await RawSql.VisibleAccountIdsAsync(db,userId,fullWorthSpaceId,ct); if(visible.Count==0) return Results.Ok(Array.Empty<IncomeCandidate>());
    ...
    private static async Task<IResult> AcceptCandidate(Guid fullWorthSpaceId, IncomeCandidate request, CurrentUserContext currentUser, FullWorthDbContext db, AuditService audit, CancellationToken ct)
    {
        ...
        var id=Guid.NewGuid(); var connection=await RawSql.OpenAsync(db,ct); var now=DateTimeOffset.UtcNow;
...
""",("@id",id),("@space",fullWorthSpaceId),("@name",request.Counterparty.Trim()),("@account",request.AccountId),("@party",party),("@amount",request.TypicalAmount),("@currency",currency),("@cycle",cycle),("@next",request.NextExpectedDate),("@now",now)))
    ...
    private static async Task<IResult> DismissCandidate(Guid fullWorthSpaceId, IncomeCandidateDismissWrite request, CurrentUserContext currentUser, FullWorthDbContext db, AuditService audit, CancellationToken ct)
    {
        var userId=currentUser.RequireUserId();
        if(!await SpaceCapabilities.HasCapabilityAsync(db,userId,fullWorthSpaceId,"budgets.manage",ct)) return Results.StatusCode(403);
        var visible=await RawSql.VisibleAccountIdsAsync(db,userId,fullWorthSpaceId,ct); if(!visible.Contains(request.AccountId)) return Results.NotFound();
        var writable=await RawSql.WritableAccountIdsAsync(db,userId,fullWorthSpaceId,ct); if(!writable.Contains(request.AccountId)) return Results.StatusCode(403);
        var party=NormalizeCandidateParty(request.Counterparty); var cycle=NormalizeDetectedCycle(request.Cycle); var currency=request.Currency?.Trim().ToUpperInvariant();
        if(party is null||cycle is null||currency is null||currency.Length!=3) return Results.BadRequest(new{error="Invalid income candidate."});
        var connection=await RawSql.OpenAsync(db,ct);
        await using var cmd=RawSql.Command(connection,"""
...
""",("@space",fullWorthSpaceId),("@account",request.AccountId),("@party",party),("@currency",currency),("@cycle",cycle),("@now",DateTimeOffset.UtcNow));
    ...
    private static async Task<IResult> GetSettings(Guid fullWorthSpaceId, CurrentUserContext currentUser, FullWorthDbContext db, CancellationToken ct)
    {
        ...
        var connection=await RawSql.OpenAsync(db,ct);
        await using var cmd=RawSql.Command(connection,"SELECT \"HorizonMode\",\"SafetyReserveAmount\",\"SafetyReserveCurrency\",\"IncludePendingIncome\",\"IncludePendingExpenses\",\"VariableForecastMode\" FROM \"CashflowPlanSettings\" WHERE \"FullWorthSpaceId\"=@space",("@space",fullWorthSpaceId));
        ...
        return Results.Ok(new CashflowSettingsWrite(RawSql.String(r,"HorizonMode"),RawSql.Decimal(r,"SafetyReserveAmount"),RawSql.String(r,"SafetyReserveCurrency"),RawSql.Bool(r,"IncludePendingIncome"),RawSql.Bool(r,"IncludePendingExpenses"),RawSql.String(r,"VariableForecastMode")));
    }
    ...
    private static async Task<IResult> PutSettings(Guid fullWorthSpaceId, CashflowSettingsWrite request, CurrentUserContext currentUser, FullWorthDbContext db, AuditService audit, CancellationToken ct)
    {
        ...
        var connection=await RawSql.OpenAsync(db,ct); await using var cmd=RawSql.Command(connection,"""
...
""",("@space",fullWorthSpaceId),("@mode",request.HorizonMode),("@reserve",request.SafetyReserveAmount),("@currency",request.SafetyReserveCurrency.Trim().ToUpperInvariant()),("@pi",request.IncludePendingIncome),("@pe",request.IncludePendingExpenses),("@forecast",request.VariableForecastMode),("@now",DateTimeOffset.UtcNow));
    ...
    {
        var userId=currentUser.RequireUserId(); if(!await RawSql.IsMemberAsync(db,userId,fullWorthSpaceId,ct))return Results.NotFound();
        var visible=await RawSql.VisibleAccountIdsAsync(db,userId,fullWorthSpaceId,ct); var day=asOf??DateOnly.FromDateTime(DateTime.UtcNow);
        ...
        var contracts=await db.Contracts.AsNoTracking().Where(c=>c.FullWorthSpaceId==fullWorthSpaceId&&c.IsActive&&c.CountsAsFixedCost&&c.MergedIntoContractId==null&&c.NextDueDate>=day&&c.NextDueDate<=horizon&&(c.AccountId==null||visible.Contains(c.AccountId.Value))).ToListAsync(ct); decimal fixedCosts=0;
