...
/// <summary>
/// Ein erwarteter Geldeingang, soweit die Planung ihn braucht.
///
/// Die letzten vier Felder kamen fuer die Zukunfts-Timeline (#139) dazu - angehaengt, nicht
/// eingeschoben: der einzige bestehende Aufrufer (<c>CashflowEndpoints.GetAvailable</c>) liest per
/// Name, nicht positional, bleibt also unberuehrt.
/// </summary>
public sealed record ScheduleRow(
    Guid Id, string Name, Guid? AccountId, decimal? Amount, string Currency, DateOnly? NextDate,
    string? NormalizedCounterparty, string Cycle, int Interval, string ValueMode);
