...
| `alpha.21` | bAV step 3: the wealth block, the salary link, the dashboard entry, the projection and the variant comparison. Plus the forward preview (contracts, budgets and income instead of the net-worth curve, a fixed-cost tick per contract, an expected increase per line, purchasing power) and the last three dialogs of [UI_AUDIT.md](UI_AUDIT.md) — budgets, contract cancellation, property valuation. | shipped |
| `alpha.36` | The installation's own settings: everything that used to mean editing YAML and restarting is in the admin menu and applies without a restart — Enable Banking, FinTS, sync, registration and the log level. The Codex bridge moved into the application container under its own user and only starts when somebody signs in. The deploy compose is 78 lines instead of 177 and the public one needs no variables at all. And the secret vault: an administrator can see the stored credentials and infrastructure secrets, behind a second factor. | Change a setting in *Admin → Einstellungen dieser Installation* and watch it take effect without a restart; upgrade with `--remove-orphans` and check the Codex sign-in survived; open *Passwörter & Schlüssel* and confirm it asks for a code before it shows anything |
